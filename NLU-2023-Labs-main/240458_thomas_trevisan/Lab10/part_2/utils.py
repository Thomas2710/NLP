...
)  # cuda:0 means we are using the GPU with id 0, if you have multiple GPU
...
def create_splits(tmp_train_raw, test_raw):
    ...
    return train_raw, dev_raw, intents, y_test
...
class Lang:
    ...
    def w2id(self, elements, cutoff=None, unk=True):
        ...
        vocab[CLS_TOKEN] = CLS_ID
        vocab[SEP_TOKEN] = SEP_ID
        if unk:
...
class IntentsAndSlots(data.Dataset):
    ...
    def __getitem__(self, idx):
        utt = torch.Tensor(
            tokenizer.build_inputs_with_special_tokens(self.utt_ids[idx])
        )
